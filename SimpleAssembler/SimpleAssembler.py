...
def take_input()->list:         #Function to take input and check for any possible errors
    ...
    for num, line in inpt:
        ...
        if pos_types[0] == 'F':
            halt_obs = True
    if not(halt_obs) and (errors['Halt instruction not at the end'] == []):
        errors['Missing halt instruction'].append(len(raw_inpt))
    ...
    return [line[1] for line in raw_inpt]
...
def convert(inpt:list)->list:
    ...
    for i in code:
        ...
        if(lst[x] in A): #A type
            ...
            if(B.get(lst[x+0])=="00010"):
                s+=dec_2_bin(float(lst[x+2][1::]))
            else:
